...
export default Vue.extend({
  ...
  data() {
    const returnable: Idata = {
      urls: [
        { link: "https://www.youtube.com/watch?v=HZcic7lEcmU&list=PLRqwX-V7Uu6ZRrqLcQ5BkBKmBLiGD8n4O&index=4", title: "Friction Force" },
        { link: "https://www.youtube.com/watch?v=YvNiLmHXZ_U&list=PLRqwX-V7Uu6ZRrqLcQ5BkBKmBLiGD8n4O&index=5", title: "Drag Force" },
      ],
      movers: [],
    };
    return returnable;
  ...
  methods: {
    ...
    createNewMover(ctx: P5Sketch): Mover {
      ...
      const location = ctx.createVector(randomXCord, 0);
    ...
    update(ctx: P5Sketch, mover: Mover) {
      ...
      if (ctx.mouseIsPressed) {
        this.applyFrictionForce(ctx, mover);
      }

      if (mover.location.y >= ctx.height * 0.5) {
        this.applyDragForce(ctx, mover);
      }
    ...
    applyFrictionForce(ctx: P5Sketch, mover: Mover) {
      // Force friction = -1 * unit of velocity * co-effiicience of magnitude of normal force
      const friction = cloneDeep(mover.velocity);
      const coEffOfNormalForce = 10; // co-effiicience of magnitude of normal force

      friction.normalize(); // unit of velocity
      friction.mult(-1);
      friction.mult(coEffOfNormalForce);

      this.applyNewtonsSecondLaw(friction, mover);
    },
    applyDragForce(ctx: P5Sketch, mover: Mover) {
      // drag friction = 0.5 * density of env * (magnitude of velocity * magnitude of velocity) * surface area of object *  coEff of drag * unit of vector
      const drag = cloneDeep(mover.velocity);
      drag.normalize(); // unit of vector
      const coEffofDrag = -1;
      const magnitudeSquared = mover.velocity.magSq(); // magnitude of velocity
      // NOTE: density & surface area of object is not present, hence assumed 1
      drag.mult(coEffofDrag * magnitudeSquared * 0.5);
      this.applyNewtonsSecondLaw(drag, mover);
    },
  },
});
</script>
